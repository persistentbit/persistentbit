...
public class DbNameTransformer {
	...
	public DbNameTransformer(Function<String,String> generalDbNameToJava){
	    this(cat -> "c_" + generalDbNameToJava.apply(cat.getName().orElse("catalog")).toLowerCase(), schema -> "s_" + generalDbNameToJava.apply(schema.getName().orElse("schema")).toLowerCase(), table -> generalDbNameToJava.apply(table.getName()), (tab, col) -> UString
			.firstLowerCase(generalDbNameToJava.apply(col.getName())), generalDbNameToJava);
	}
	@Generated
