...
CREATE VIEW goatada_test.V_EXC_COLL_FEED_INFO AS
  SELECT `goatada_test `.`COLL_PROVIDER_FEED_STATUS`.`FEED_IDENTIFIER_CODE` AS `FEED_IDENTIFIER_CODE`,
  `goatada_test`.`COLL_PROVIDER_FEED_STATUS`.`DATA_PROVIDER_CODE` AS `DATA_PROVIDER_CODE`,
...
CREATE VIEW goatada_test.V_SUBJECT_STATUS AS
  ...
  `it`.`TAG_VALUE` AS `CLASS_TAG`,
( CASE `i`.`VALIDATION_STATUS`
WHEN 'INITIAL'
THEN 1
ELSE 1000 END ) AS `VALIDATION_STATUS_ORDER`
FROM (`goatada_test`.`FIMA_FINANCIAL_INSTRUMENT` `i` LEFT JOIN `goatada_test`.`FIMA_FINANCIAL_INSTR_TAG` `it`
ON (((`it`.`TAG_TYPE` = 'CLASSIFICATION') AND
(`i`.`INSTRUMENT_IDENTIFICATION_CODE` = `it`.`INSTRUMENT_IDENTIFICATION_CODE`))))
UNION SELECT concat('P:', `p `.`PARTY_IDENTIFICATION_CODE`) AS `SUBJECT_ID`,
`p`.`VALIDATION_STATUS` AS `VALIDATION_STATUS`,
`p`.`FOLLOW_UP_STATUS` AS `FOLLOW_UP_STATUS`,
NULL AS `CLASS_TAG`,
( CASE `p`.`VALIDATION_STATUS`
WHEN 'INITIAL'
THEN 1
ELSE 1000 END ) AS `VALIDATION_STATUS_ORDER`
FROM `goatada_test`.`FIMA_PARTY ` ` p `;
